...
#define TEST_CASE_MY_MAIN(name, ...)                         \
  ...
  static void name();                                        \
  namespace {                                                \
    jdl::registration UNIQUE_NAME_LINE(reg, __LINE__)(name); \
  }                                                          \
  STOP_WARNINGS_SUPPRESSION                                  \
  void name()

#define TEST_CASE_MY(...) TEST_CASE_MY_MAIN(UNIQUE_NAME_LINE(test_case_, __LINE__), __VA_ARGS__)
  ...
  {                                                                                        \
    if ((x).compare((y)) != 0) {                                                           \
      throw std::runtime_error(std::string(__FILE__) + std::string(":") +                  \
                               std::to_string(__LINE__) + std::string(" in ") +            \
                               std::string(__PRETTY_FUNCTION__) + std::string(": ") +      \
                               std::string((x)) + std::string(" != ") + std::string((y))); \
    }                                                                                      \
  }
